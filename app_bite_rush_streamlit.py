...
DATA_DIR = Path(r"D:\Internship Analytixlabs\Project 2\Datasets\BiteRush - Food delivery datasets")
...
def load_model_bundle():
    ...
    if not isinstance(bundle, dict) or "pipeline" not in bundle or "feature_cols" not in bundle:
        ...
        return {"pipeline": repaired, "feature_cols": None, "delay_threshold": DEFAULT_DELAY_THRESHOLD, "model_name": "Model"}
    bundle["pipeline"] = repair_sklearn_tree_attributes(bundle["pipeline"])
    bundle.setdefault("delay_threshold", DEFAULT_DELAY_THRESHOLD)
    bundle.setdefault("model_name", "Model")
    return bundle
